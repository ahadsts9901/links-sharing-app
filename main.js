        ...
        .then(function(querySnapshot) {
            if (querySnapshot.size === 0) {
                ...
                querySnapshot.forEach(function(doc) {
                    ...
                    row.className = "rowScroll"; // Use assignment instead of concatenation
                    section.appendChild(row);

                    let link = document.createElement('a');
                    link.className = "scrollX"; // Use assignment instead of concatenation
                    link.setAttribute("target", "_blank");
                    link.setAttribute("href", `${data.link}`); // Use data.link instead of undefined input variable
                    link.innerText = data.link; // Use data.link instead of undefined input variable
                    row.appendChild(link);

                    let del = document.createElement('p');
                    del.className = "bi bi-trash-fill"; // Use assignment instead of concatenation
                    del.addEventListener("click", () => deleteLink(doc.id)); // Pass the document ID to deleteLink function
                    row.appendChild(del);
                });
            }
        })
